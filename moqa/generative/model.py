...
class MT5QA(T5PreTrainedModel):
    ...
    def encoder_cache_wrapper(self, input_ids, *args, **kwargs):
        encoder_outputs = kwargs.pop("encoder_outputs", None)
        if encoder_outputs is not None:
            return encoder_outputs
        else:
            return self.encoder(input_ids, *args, **kwargs)

    def get_encoder(self):
        return self.encoder_cache_wrapper
    ...
    def forward(
        ...
        if not return_dict:
            output = (lm_logits,) + decoder_outputs[1:] + concatenated_encoder_outputs  # Renamed (MF)
    ...
    def concatenate_encoder_outputs(self,
        ...
        hidden_states = hidden_states.view(-1, hidden_states.shape[-1])[mask].unsqueeze(0)
